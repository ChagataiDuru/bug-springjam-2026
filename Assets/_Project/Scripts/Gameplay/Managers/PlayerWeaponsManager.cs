...
{
    /// <summary>
    /// Manages player weapon inventory, switching, aiming, recoil, and bob animations.
    /// </summary>
    /// <remarks>
    /// Handles up to 9 weapon slots. Manages weapon switching animations, aim-down-sights,
    /// FOV transitions, and procedural weapon movement.
    /// </remarks>
    [RequireComponent(typeof(PlayerInputHandler))]
    public class PlayerWeaponsManager : MonoBehaviour
    {
        /// <summary>States for the weapon switching animation.</summary>
        ...
        NetworkedPlayerController m_NetworkedPlayerController;
        NetworkedWeaponDriver m_NetworkedWeaponDriver;
        bool m_EnableLegacyDirectFirePath;
        ...
        void Start()
        {
            ...
            m_NetworkedPlayerController = GetComponent<NetworkedPlayerController>();
            m_NetworkedWeaponDriver = GetComponent<NetworkedWeaponDriver>();

            // Card 1 transition guard:
            // keep direct fire routing only for legacy non-network scenes that do not use the driver.
            m_EnableLegacyDirectFirePath = m_NetworkedPlayerController == null && m_NetworkedWeaponDriver == null;

            if (m_NetworkedPlayerController != null && m_NetworkedWeaponDriver == null)
            {
                Debug.LogWarning(
                    "[PlayerWeaponsManager] Networked player is missing NetworkedWeaponDriver. Direct-fire fallback is disabled.");
            }
        ...
        void Update()
        {
            ...
            if (activeWeapon != null && m_WeaponSwitchState == WeaponSwitchState.Up)
            {
                ...
                if (m_EnableLegacyDirectFirePath)
                {
                    // Legacy compatibility path for old local-only scenes.
                    ProcessPredictedPrimaryUseInput(
        ...
        /// <summary>
        /// Card 2 local predicted/presentation path used by NetworkedWeaponDriver.
        /// This remains presentation-only; authoritative shot gameplay resolves on host.
        /// </summary>
        public bool ProcessPredictedPrimaryUseInput(bool inputDown, bool inputHeld, bool inputUp)
        {
            WeaponController activeWeapon = GetActiveWeapon();
            if (!CanProcessUseInput(activeWeapon))
            {
                return false;
            }

            bool hasFired = activeWeapon.PlayLocalPredictedUseInput(inputDown, inputHeld, inputUp);
            if (hasFired)
            {
                ApplyWeaponRecoil(activeWeapon);
            }

            return hasFired;
        }

        public void StopPredictedUseOnWeapon(WeaponController weapon)
        {
            if (weapon == null)
            {
                return;
            ...
            weapon.PlayLocalPredictedUseStop();
        }

        public bool CanProcessUseInput(WeaponController weapon)
        {
            return weapon != null &&
        ...
        public void ApplyWeaponRecoil(WeaponController weapon)
        {
            if (weapon == null)
            {
                return;
        ...
        void UpdateWeaponSwitching()
        {
            ...
            if (switchingTimeFactor >= 1f)
            {
                if (m_WeaponSwitchState == WeaponSwitchState.PutDownPrevious)
                {
                    ...
                    if (oldWeapon != null)
                    {
                        oldWeapon.PlayLocalPredictedUseStop();
        ...
        public bool RemoveWeapon(WeaponController weaponInstance)
        {
            ...
            for (int i = 0; i < m_WeaponSlots.Length; i++)
            {
                ...
                if (m_WeaponSlots[i] == weaponInstance)
                {
                    ...
                    weaponInstance.PlayLocalPredictedUseStop();
