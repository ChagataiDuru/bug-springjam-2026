...
{
    /// <summary>
    /// Standard projectile with physics-based movement, hit detection, and damage application.
    /// </summary>
    /// <remarks>
    /// Supports trajectory correction for FPS accuracy, gravity, and area-of-effect damage.
    /// Card 2 note: player-owned networked shots use host-authoritative trace in NetworkedWeaponDriver.
    /// This projectile remains for legacy/local full simulation and AI-compatible paths.
    /// </remarks>
    public class ProjectileStandard : ProjectileBase
    {
        ...
        {
            // Card 4: networked player shots are gameplay-authoritative through NetworkedWeaponDriver/host resolver.
            // Legacy projectile gameplay damage remains for non-networked or AI-compatible paths.
            if (!ShouldSuppressGameplayDamageMutation())
            {
                if (AreaOfDamage)
                {
                    // area damage
        ...
        bool ShouldSuppressGameplayDamageMutation()
        {
            ...
            GameObject ownerObject = m_ProjectileBase.Owner;
            return ownerObject.GetComponent<NetworkedWeaponDriver>() != null ||
                   ownerObject.GetComponentInParent<NetworkedWeaponDriver>() != null ||
                   ownerObject.GetComponentInChildren<NetworkedWeaponDriver>() != null;
        }
