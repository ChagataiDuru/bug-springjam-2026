...
{
    /// <summary>
    /// Core enemy behavior controller handling navigation, combat, visual feedback, and death.
    /// </summary>
    /// <remarks>
    /// Requires Health, Actor, and NavMeshAgent. Manages patrol pathing, weapon handling,
    /// damage reactions (eye color, body flash), and loot drops. Registers with EnemyManager on start.
    /// </remarks>
    [RequireComponent(typeof(Health), typeof(Actor), typeof(NavMeshAgent))]
    public class EnemyController : MonoBehaviour, IEnemyHitReactionHost
    {
        /// <summary>Pairs a renderer with a material index for property block updates.</summary>
        [System.Serializable]
        ...
        MonoBehaviour m_GameSessionManager;
        FieldInfo m_GameSessionCurrentStateField;
        PropertyInfo m_SyncVarValueProperty;
        ...
        // Card 5: stun state managed by IEnemyHitReactionHost callbacks from NetworkedEnemySync.
        bool m_IsStunned;

        // ── IEnemyHitReactionHost ─────────────────────────────────────────────────

        /// <inheritdoc/>
        public bool IsDeadOrDying => m_Health != null && m_Health.CurrentHealth <= 0f;

        /// <inheritdoc/>
        /// <remarks>
        /// Set by <see cref="OnHitReactionStunBegin"/> / <see cref="OnHitReactionStunExpired"/>,
        /// which are called on both host and clients by NetworkedEnemySync via the SyncVar mirror.
        /// EnemyMobile reads this every frame to gate AI state machine execution.
        /// </remarks>
        public bool IsStunned => m_IsStunned;

        /// <inheritdoc/>
        public void OnHitReactionKnockbackBegin()
        {
            // Animation / VFX hook — no-op for MVP.
            // EnemyMobile.OnDamaged (triggered by Health) already fires the damage anim on host.
        }

        /// <inheritdoc/>
        public void OnHitReactionKnockbackEnd()
        {
            // NavAgent restart is handled entirely by NetworkedEnemySync.
        }

        /// <inheritdoc/>
        public void OnHitReactionStunBegin()
        {
            m_IsStunned = true;
        }

        /// <inheritdoc/>
        public void OnHitReactionStunExpired()
        {
            m_IsStunned = false;
        }
        ...
        /// <inheritdoc/>
        /// <remarks>
        /// Called on observer clients via ObserversRpc. Provides visual/audio feedback
        /// since Health.TakeDamage (and therefore OnDamaged) only fires on the host.
        /// Must NOT mutate any gameplay-truth state.
        /// </remarks>
        public void PlayHitReactionPresentation(Vector3 hitPoint, Vector3 knockbackDirection)
        {
            onDamaged?.Invoke();
        ...
        void Start()
        {
            ...
            {
                m_GameSessionManager = FindSessionManagerByTypeName("GameSessionManager");
                CacheGameSessionStateAccessors();

                if (m_GameSessionManager == null)
                {
                    Debug.LogWarning(
                        "[EnemyController] Neither GameFlowManager nor GameSessionManager found. Enemy attacks will continue until a session manager is available.");
                }
            }
        ...
        bool IsSessionEnding()
        {
            ...
            if (m_GameSessionManager == null || m_GameSessionCurrentStateField == null || m_SyncVarValueProperty == null)
            {
                return false;
            }

            object syncVar = m_GameSessionCurrentStateField.GetValue(m_GameSessionManager);
            if (syncVar == null)
            {
                return false;
            }

            object stateValue = m_SyncVarValueProperty.GetValue(syncVar, null);
            if (stateValue == null)
            {
                return false;
            }
            ...
            return stateName == "SessionWin" || stateName == "SessionFail";
        }

        MonoBehaviour FindSessionManagerByTypeName(string typeName)
        {
        ...
        void CacheGameSessionStateAccessors()
        {
            m_GameSessionCurrentStateField = null;
            m_SyncVarValueProperty = null;
            ...
            System.Type managerType = m_GameSessionManager.GetType();
            m_GameSessionCurrentStateField = managerType.GetField("CurrentState");
            if (m_GameSessionCurrentStateField == null)
            {
                return;
            }
            ...
            m_SyncVarValueProperty = syncVar.GetType().GetProperty("value");
        }
