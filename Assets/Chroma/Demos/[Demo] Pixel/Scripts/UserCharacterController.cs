...
public class UserCharacterController : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        if (Mathf.Abs(currentVelocity.x) <= maxSpeed) {
            ...
            rb.AddForce(new Vector2(((input.x * speed) - currentVelocity.x) * a, 0f));
        }

        // Stop or move player.
        float vx = (Math.Abs(input.x) < float.Epsilon && groundState.IsGround()) ? 0 : currentVelocity.x;
        // Jump from ground.
        float vy = (Math.Abs(input.y - 1) < float.Epsilon && groundState.IsTouching()) ? jumpPower : currentVelocity.y;
